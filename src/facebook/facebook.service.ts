...
export class FacebookService {
  ...
  async postStatus(createFacebookPostDto: CreateFacebookPostDto) {
    ...
    const response = await axios.post<{}, PostStatus>(
      ...
      if (error instanceof AxiosError) {
        const err = new CreateFacebookResponseErrDto(error.response.status, true, error.response.data);
        return err
      }
    });
    ...
    await writeFile(`${formatFilenameWithTimestamp('json')}`, JSON.stringify(response.data));
